...
public class Staff_Enroll_Professor extends JFrame {
	...
	public Staff_Enroll_Professor() {
		...
		enroll_year.setModel(new DefaultComboBoxModel(new String[] { "임용년도", "2022", "2021", "2020",
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (pro_id.getText().length() == 0) {
					...
					try {
						...
						new Alert("교번은 숫자만 입력이 가능합니다.");
						return;
					}
					// 2. check length
					if (pro_id.getText().length() != 9) {
						new Alert("교번은 9자리입니다.");
						return;
					}
		...
		back_btn.setFont(new Font("맑은 고딕", Font.BOLD, 14));
